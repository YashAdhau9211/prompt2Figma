...
def validate_component_structure(node: dict, path: str = "root") -> list:
        ...
        if isinstance(node["children"], list):
            for i, child in enumerate(node["children"]):
                if isinstance(child, str):
                    issues.append(
                        f"{path}.children[{i}]: Text component has string in children array: '{child}'"
                    )
                elif not isinstance(child, dict):
                    issues.append(
                        f"{path}.children[{i}]: Children array contains non-object value"
                    )
    ...
    if "children" in node and isinstance(node["children"], list):
        for i, child in enumerate(node["children"]):
            if isinstance(child, str):
                issues.append(
                    f"{path}.children[{i}]: Children array contains string: '{child}'"
                )
            elif isinstance(child, dict):
                # Recursively validate children
                child_path = (
                    f"{path}.children[{i}]({child.get('componentName', 'unnamed')})"
                )
                issues.extend(validate_component_structure(child, child_path))
            elif child is not None:
                issues.append(
                    f"{path}.children[{i}]: Children array contains invalid type: {type(child)}"
                )
...
def generate_wireframe_json(prompt: str) -> dict:
    ...
    try:
        ...
        if isinstance(json_output, list):
            logging.warning(
                "TASK 1: Model returned an array, wrapping in root container"
            )
            json_output = {
        ...
        if validation_issues:
            logging.warning(
                f"TASK 1: Component structure issues detected after sanitization: {validation_issues}"
            )
        else:
            logging.info("TASK 1: Component structure validation passed.")

        logging.info(
            "TASK 1: Successfully generated and normalized wireframe JSON for Figma compatibility."
        )
        return json_output
...
def generate_react_code(wireframe_json: dict) -> str:
    ...
    try:
        ...
        response = model.generate_content(
            f"Here is the detailed JSON UI structure:\n{json_input_str}"
        )
...
def validate_code_ast(react_code: str) -> dict:
    ...
    try:
        ...
        logging.info(
            f"TASK 3 [validate_code_ast]: Raw output from validator: '{result_json_str}'"
        )
        ...
        if process.stderr:
            ...
            if stderr_output.strip():
                logging.error(
                    f"AST validation script produced an error: {stderr_output}"
                )
        ...
        if not result_json_str.strip():
            ...
            return {
                ...
                "errors": ["Validator returned empty output."],
            }
        ...
        logging.info(
            f"TASK 3 [validate_code_ast]: Validation complete. Status: {validation_result.get('validation_status')}"
        )
        return validation_result

    except FileNotFoundError:
        logging.error(
            "AST validation script not found or 'node' command is not available."
        )
        return {
            "code": react_code,
            "validation_status": "FAILURE",
