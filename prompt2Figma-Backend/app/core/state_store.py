...
class RedisStateStore:
    ...
    async def connect(self):
        ...
        if self._redis is None:
            try:
                ...
                logger.info(f"Connected to Redis state store (pool: {self.use_connection_pool})")
    ...
    async def _execute_with_circuit_breaker(self, operation, *args, **kwargs):
        ...
        try:
            ...
            if self.circuit_breaker.state.value == "open":
                self.degradation_manager.enable_degraded_mode("Redis operation failures")
            raise
    ...
    async def get_design_state(self, session_id: str, version: Optional[int] = None) -> Optional[DesignState]:
        ...
        async with PerformanceTimer(
            ...
            try:
                ...
                if self.degradation_manager.is_degraded():
                    logger.warning(f"Operating in degraded mode for session {session_id}")
                    cached_data = await self.degradation_manager.get_cached_session(session_id)
                ...
                is_valid, error_msg = await self.recovery_manager.validate_session_state(
                    session_id, data
                )
                
                if not is_valid:
                    logger.warning(f"Invalid state detected for {session_id} v{version}: {error_msg}")
                    ...
                    for prev_version in previous_versions:
                        if prev_version < version:
                            prev_data = await self.redis.hgetall(f"session:{session_id}:state:v{prev_version}")
                            if prev_data:
                    ...
                    if recovered_state:
                        logger.info(f"Successfully recovered state for {session_id} v{version}")
                        # Store recovered state back to Redis
                        await self.store_design_state(session_id, version, recovered_state)
                        return recovered_state
                    else:
                        logger.error(f"Failed to recover state for {session_id} v{version}")
                        return None
                
                # State is valid, convert to model
                # Use efficient deserialization if pool manager available
                if self.pool_manager:
                    wireframe_json = self.pool_manager.deserialize(data["wireframe_json"])
                ...
                is_model_valid, model_error = await self.recovery_manager.validate_design_state_model(
                    design_state
                )
                ...
                logger.error(f"Circuit breaker prevented state retrieval for {session_id}")
                return None
            except Exception as e:
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "healthy": (
                circuit_stats["state"] == "closed" and
                not degradation_status["degraded_mode"]
            )
        }
