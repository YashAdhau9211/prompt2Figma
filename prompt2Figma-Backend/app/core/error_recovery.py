...
class SessionRecoveryManager:
    ...
    def _build_validation_rules(self) -> Dict[str, callable]:
        ...
        return {
            ...
            "wireframe_is_json": lambda data: self._is_valid_json(data.get("wireframe_json", "")),
            "metadata_is_json": lambda data: self._is_valid_json(data.get("metadata", "")),
    ...
    async def recover_session_state(
        ...
        if repaired_state:
            logger.info(f"Successfully repaired corrupted state for session {session_id}")
    ...
    async def _attempt_repair(self, corrupted_data: Dict[str, Any]) -> Optional[DesignState]:
        """
        ...
        try:
            ...
            if "wireframe_json" in repaired:
                if isinstance(repaired["wireframe_json"], str):
                    try:
                        repaired["wireframe_json"] = json.loads(repaired["wireframe_json"])
            ...
            design_state = DesignState(
                wireframe_json=repaired["wireframe_json"] if isinstance(repaired["wireframe_json"], dict) else json.loads(repaired["wireframe_json"]),
                metadata=repaired["metadata"] if isinstance(repaired["metadata"], dict) else json.loads(repaired["metadata"]),
                created_at=datetime.fromisoformat(repaired["created_at"]) if isinstance(repaired["created_at"], str) else repaired["created_at"],
...
class GracefulDegradationManager:
    ...
    def disable_degraded_mode(self):
        ...
        if self._degraded_mode:
            duration = (datetime.utcnow() - self._degradation_start_time).total_seconds()
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "utilization": len(self._in_memory_cache) / self._cache_size if self._cache_size > 0 else 0,
