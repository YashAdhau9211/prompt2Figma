...
class SummarizationStrategy(ContextCompressionStrategy):
    ...
    def _create_summary_context(self, contexts: List[EditContext]) -> EditContext:
        ...
        avg_processing_time = sum(ctx.processing_time_ms for ctx in contexts) / len(
            contexts
        )
        ...
        summary_prompt = (
            f"[Summary of {len(contexts)} older edits: {', '.join(summary_parts)}]"
        )
...
class AdaptiveCompressionStrategy(ContextCompressionStrategy):
    ...
    def __init__(self, max_size: int = 15, importance_threshold: float = 0.7):
        """
...
class ContextCompressor:
    ...
    def get_compression_ratio(
        self, original: List[EditContext], compressed: List[EditContext]
    ) -> float:
        """
...
def compress_context_window(
    contexts: List[EditContext], max_size: int = 10
) -> List[EditContext]:
    """
...
def compress_with_summarization(
    contexts: List[EditContext], detailed_window: int = 5, summary_window: int = 20
) -> List[EditContext]:
    """
