...
from app.core.models import DesignState, EditContext, EditType, ProcessedEdit
...
"""
    ...
    async def process_contextual_edit(
        ...
        try:
            ...
            if processed_edit.needs_clarification:
                logger.warning(
                    f"Edit requires clarification: {processed_edit.clarification_options}"
                )
            ...
            processed_edit.processing_metadata["total_processing_time_ms"] = (
                processing_time
            )
            processed_edit.processing_metadata["gemini_call_success"] = True

            logger.info(
                f"Successfully processed contextual edit in {processing_time}ms"
            )
    ...
    def _build_gemini_prompt(
        self, processed_edit: ProcessedEdit, current_design: Dict[str, Any]
    ) -> str:
        """
        ...
        if processed_edit.target_elements:
            context_info.append(
                f"Target Elements: {', '.join(processed_edit.target_elements)}"
            )
