...
from app.core.models import (DesignState, EditContext, EditIntent, EditType, IterativeDesignError,
                             ProcessedEdit)
...
class ContextProcessingEngine:
    ...
    def _build_reference_patterns(self) -> Dict[ReferenceType, List[str]]:
        ...
        return {
            ReferenceType.PRONOUN: [r"\b(it|that|this)\b", r"\b(them|those|these)\b"],
    ...
    async def process_edit_with_context(
        ...
        """
        Process an edit prompt with full context awareness and graceful error handling.
        ...
        try:
            # Step 1: Extract basic intent (with fallback)
            try:
            ...
            # Step 2: Resolve references in the prompt (with fallback)
            try:
                resolved_elements, confidence = await self._resolve_references(
                    edit_prompt, current_state, context_history
                )
            except Exception as e:
                logger.warning(f"Reference resolution failed, using fallback: {e}")
                resolved_elements = []
                confidence = 0.0

            # Step 3: Build enhanced prompt for AI model (with fallback)
            try:
            ...
            if needs_clarification:
                try:
                    ...
                    clarification_options = [
                        "Please provide more specific details about what you want to change."
                    ]
            ...
            return ProcessedEdit(
                ...
                processing_metadata={
                    ...
                    "elements_in_design": (
                        len(
                            self._extract_elements_from_design(
                                current_state.wireframe_json
                            )
                        )
                        if current_state
                        else 0
                    ),
                    "degraded_mode": confidence == 0.0 and not resolved_elements,
                },
            )

        except Exception as e:
            logger.error(f"Critical error processing edit with context: {e}")
            ...
            return ProcessedEdit(
                ...
                clarification_options=[
                    "An error occurred while processing your request. Please try rephrasing your edit or specify which element you want to modify."
                ],
                processing_metadata={
                    "error": str(e),
                    "processing_time_ms": processing_time,
    ...
    def extract_edit_intent(self, prompt: str) -> EditIntent:
        ...
        if (
            any(
                ...
                for word in ["bigger", "smaller", "large", "small", "tiny", "huge"]
            )
            or "size" in prompt_lower
        ):
            return EditIntent.CHANGE_SIZE
        # Check for text changes with quotes
        elif (
            any(word in prompt_lower for word in ["say", "text", "label", "title"])
            and '"' in prompt
        ) or ("text" in prompt_lower and "to" in prompt_lower):
    ...
    async def _resolve_references(
        ...
        design_elements = self._extract_elements_from_design(
            current_state.wireframe_json
        )
        ...
        for element_type in element_matches:
            if element_type in self._element_types:
                ...
                matching_elements = [
                    ...
                    if element_type in elem.get("type", "").lower()
                    or element_type in elem.get("id", "").lower()
                    or element_type in elem.get("class", "").lower()
                ]
                ...
                if matching_elements:
                    ...
                    if len(matching_elements) == 1:
                        resolved_elements.append(
                            matching_elements[0].get("id", element_type)
                        )
                        confidence_scores.append(0.9)
                    else:
                        # Multiple matches - lower confidence
                        resolved_elements.extend(
                            [elem.get("id", element_type) for elem in matching_elements]
                        )
    ...
    def build_contextual_prompt(
        ...
        if recent_changes:
            ...
            for i, change in enumerate(recent_changes[:3]):  # Last 3 changes
                context_parts.append(
                    f"{i+1}. {change.prompt} (type: {change.edit_type})"
                )
...
"""
    ...
    def _extract_elements_from_design(
        self, design_json: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """
    ...
    def _get_recent_target_elements(
        self, context_history: List[EditContext]
    ) -> List[str]:
        ...
        sorted_contexts = sorted(
            context_history, key=lambda x: x.timestamp, reverse=True
        )
    ...
    async def _generate_clarification_options(
        self, prompt: str, current_state: DesignState, resolved_elements: List[str]
    ) -> List[str]:
        ...
        if len(resolved_elements) > 1:
            options.append(
                f"Which element do you want to modify: {', '.join(resolved_elements)}?"
            )
        ...
        # If no clear target found
            ...
            if design_elements:
                element_types = list(
                    set(elem.get("type", "element") for elem in design_elements[:5])
                )
                options.append(
                    f"Which element do you want to modify? Available: {', '.join(element_types)}"
                )
            else:
        ...
        if intent == EditIntent.UNCLEAR:
            options.append(
                "What would you like to do? (add, remove, modify, change style, etc.)"
            )

        return (
            options
            if options
            else ["Please provide more specific details about what you want to change."]
        )
