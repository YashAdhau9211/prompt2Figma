    ...
    async def check_rate_limit(self, session_id: str) -> Tuple[bool, str]:
        ...
        async with self._lock:
            ...
            if len(history) >= self.max_requests_per_day:
                logger.warning(f"Rate limit exceeded for session {session_id}: daily limit")
            ...
            if len(recent_hour) >= self.max_requests_per_hour:
                logger.warning(f"Rate limit exceeded for session {session_id}: hourly limit")
            ...
            if len(recent_minute) >= self.max_requests_per_minute:
                logger.warning(f"Rate limit exceeded for session {session_id}: minute limit")
...
class InputSanitizer:
    ...
    def sanitize_prompt(cls, prompt: str) -> Tuple[str, List[str]]:
        ...
        if len(prompt) > cls.MAX_PROMPT_LENGTH:
            raise SecurityError(f"Prompt exceeds maximum length of {cls.MAX_PROMPT_LENGTH} characters")
        
        # Check for SQL injection patterns
        for pattern in cls.SQL_INJECTION_PATTERNS:
            if re.search(pattern, prompt, re.IGNORECASE):
                raise SecurityError("Prompt contains potentially malicious SQL patterns")
        
        # Check for script injection patterns
        for pattern in cls.SCRIPT_INJECTION_PATTERNS:
            if re.search(pattern, prompt, re.IGNORECASE):
                raise SecurityError("Prompt contains potentially malicious script patterns")
    ...
    def sanitize_user_id(cls, user_id: str) -> str:
        ...
        if len(user_id) > cls.MAX_USER_ID_LENGTH:
            raise SecurityError(f"User ID exceeds maximum length of {cls.MAX_USER_ID_LENGTH}")
    ...
    def sanitize_session_id(cls, session_id: str) -> str:
        ...
        if len(session_id) > cls.MAX_SESSION_ID_LENGTH:
            raise SecurityError(f"Session ID exceeds maximum length of {cls.MAX_SESSION_ID_LENGTH}")
