...
from app.core.models import (DesignSession, DesignState, EditContext, EditResult, EditType,
                             SessionMetadata, SessionMetrics, SessionStatus, VersionDiff)
...
class DesignSessionManager:
    ...
    def __init__(self, state_store: RedisStateStore, session_timeout_hours: int = 24):
        ...
        self.prompt_processor = EnhancedPromptProcessor()
            ...
            try:
                ...
                if not success:
                    raise SessionManagerError(
                        f"Failed to store session {session.session_id} in Redis"
                    )

                logger.info(
                    f"Created new design session {session.session_id} for user {user_id}"
                )
                return session
    ...
    async def update_session_state(
        self, session_id: str, new_state: DesignState
    ) -> bool:
        """
        ...
        try:
            ...
            if session.status != SessionStatus.ACTIVE:
                raise SessionManagerError(
                    f"Cannot update inactive session {session_id}"
                )
            ...
            if not success:
                raise SessionManagerError(
                    f"Failed to store design state for session {session_id}"
                )
    ...
    async def add_edit_context(self, session_id: str, context: EditContext) -> bool:
        ...
        try:
            ...
            if not success:
                raise SessionManagerError(
                    f"Failed to add context for session {session_id}"
                )
    ...
    async def complete_session(self, session_id: str) -> bool:
        ...
        try:
            ...
            await self.state_store.redis.hset(
                session_key, "status", SessionStatus.COMPLETED.value
            )
            await self.state_store.update_session_activity(session_id)
    ...
    async def apply_edit(
        self, session_id: str, wireframe_json: dict, changes: dict, metadata: dict
    ) -> EditResult:
        """
    ...
    async def apply_contextual_edit(
        self, session_id: str, edit_prompt: str
    ) -> EditResult:
        """
        Apply a contextual edit to a session using the enhanced prompt processor.

        This method uses the context-aware prompt processing to interpret the edit
        in the context of the current design state and conversation history.

        Args:
            session_id: The session identifier
            edit_prompt: User's edit prompt (can be contextual like "make it bigger")

        Returns:
            EditResult: The result of applying the contextual edit
        ...
        async with PerformanceTimer(
            ...
            metadata={"prompt_length": len(edit_prompt)},
        ):
            try:
                # Verify session exists and is active
                session = await self.get_session(session_id)
                ...
                if session.status != SessionStatus.ACTIVE:
                    raise SessionManagerError(
                        f"Cannot edit inactive session {session_id}"
                    )

                # Get current design state
                current_state = await self.state_store.get_design_state(
                    session_id, session.current_version
                )
                if not current_state:
                    raise SessionManagerError(
                        f"Current design state not found for session {session_id}"
                    )

                # Get context history
                context_history = await self.state_store.get_context_history(
                    session_id, limit=10
                )

                # Process the contextual edit
                start_time = datetime.utcnow()
                updated_wireframe, processed_edit = (
                    await self.prompt_processor.process_contextual_edit(
                ...
                if processed_edit.needs_clarification:
                    # Return a result indicating clarification is needed
                    return EditResult(
                        success=False,
                        new_version=session.current_version,
                        updated_wireframe=current_state.wireframe_json,
                        changes_summary="Clarification needed",
                        processing_time_ms=int(
                            processed_edit.processing_metadata.get(
                                "total_processing_time_ms", 0
                            )
                        ),
                        errors=processed_edit.clarification_options,
                    )

                # Prepare changes metadata
                changes = {
                    "prompt": edit_prompt,
                    "edit_type": processed_edit.edit_type.value,
                    "target_elements": processed_edit.target_elements,
                    "summary": f"Applied {processed_edit.edit_intent.value} edit",
                    "confidence_score": processed_edit.confidence_score,
                }

                # Prepare version metadata
                version_metadata = {
                    "edit_intent": processed_edit.edit_intent.value,
                    "original_prompt": processed_edit.original_prompt,
                    "enhanced_prompt": processed_edit.enhanced_prompt,
                    "confidence_score": processed_edit.confidence_score,
                }

                # Apply the edit using the standard apply_edit method
                result = await self.apply_edit(
                    session_id, updated_wireframe, changes, version_metadata
                )
                ...
                logger.info(
                    f"Successfully applied contextual edit to session {session_id} in {total_processing_time}ms"
                )
                ...
                logger.error(
                    f"Failed to apply contextual edit to session {session_id}: {e}"
                )
                raise SessionManagerError(
                    f"Contextual edit application failed: {str(e)}"
                )
    ...
    async def get_version_diff(
        ...
        try:
            ...
            return await self.version_manager.get_version_diff(
    ...
    async def compress_session_versions(
        ...
        try:
            ...
            compressed_count = await self.version_manager.compress_old_versions(
                session_id, keep_recent
            )

            if compressed_count > 0:
                logger.info(
                    f"Compressed {compressed_count} versions for session {session_id}"
                )
    ...
    async def verify_session_integrity(self, session_id: str) -> dict:
        ...
        try:
            ...
            # Check each version
            for version in versions:
                is_valid = await self.version_manager.verify_version_integrity(
                    session_id, version
                )
                if is_valid:
    ...
    async def _mark_session_expired(self, session_id: str) -> bool:
        ...
        try:
            session_key = f"session:{session_id}:metadata"
            await self.state_store.redis.hset(
                session_key, "status", SessionStatus.EXPIRED.value
            )
            return True
        except Exception as e:
