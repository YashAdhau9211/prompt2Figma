...
from app.core.models import (
    ...
    SessionStatus, EditContext, SessionMetrics, VersionDiff, EditType
)
...
from app.core.performance_monitor import (
    get_performance_monitor, PerformanceTimer, MetricType
)
...
class DesignSessionManager:
    ...
    async def apply_contextual_edit(self, session_id: str, edit_prompt: str) -> EditResult:
        """
        ...
        async with PerformanceTimer(
            ...
            try:
                ...
                current_state = await self.state_store.get_design_state(session_id, session.current_version)
                if not current_state:
                    raise SessionManagerError(f"Current design state not found for session {session_id}")
                
                # Get context history
                context_history = await self.state_store.get_context_history(session_id, limit=10)
                
                # Process the contextual edit
                start_time = datetime.utcnow()
                updated_wireframe, processed_edit = await self.prompt_processor.process_contextual_edit(
                ...
                if processed_edit.needs_clarification:
                    ...
                    return EditResult(
                        ...
                        processing_time_ms=int(processed_edit.processing_metadata.get("total_processing_time_ms", 0)),
                ...
                result = await self.apply_edit(session_id, updated_wireframe, changes, version_metadata)
                ...
                logger.info(f"Successfully applied contextual edit to session {session_id} in {total_processing_time}ms")
                ...
                logger.error(f"Failed to apply contextual edit to session {session_id}: {e}")
                raise SessionManagerError(f"Contextual edit application failed: {str(e)}")
