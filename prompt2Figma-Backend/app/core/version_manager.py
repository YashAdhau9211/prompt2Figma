...
from app.core.models import DesignState, EditType, SessionMetrics
...
class VersionManager:
    ...
    async def create_version(
        ...
        try:
            ...
            if not success:
                raise RuntimeError(
                    f"Failed to store design state for version {new_version}"
                )

            # Store version metadata separately for quick access
            await self._store_version_metadata(
                session_id, new_version, design_state, changes
            )
    ...
    async def get_version_diff(
        ...
        try:
            ...
            from_state = await self.state_store.get_design_state(
                session_id, from_version
            )
            to_state = await self.state_store.get_design_state(session_id, to_version)

            if not from_state or not to_state:
                logger.warning(
                    f"Could not find versions {from_version} or {to_version} for session {session_id}"
                )
                return None
            ...
            logger.error(
                f"Failed to calculate diff between versions {from_version}-{to_version}: {e}"
            )
            return None
    ...
    async def compress_old_versions(
        ...
        try:
            ...
            for version in versions_to_compress:
                ...
                if success:
                    ...
                    compressed_count += 1
                    logger.info(
                        f"Compressed version {version} for session {session_id}"
                    )
        ...
        try:
            ...
            for version in all_versions[1:]:  # Skip initial version
                ...
                if metadata:
                    ...
                    edit_types_distribution[edit_type] = (
                        edit_types_distribution.get(edit_type, 0) + 1
                    )
    ...
    async def verify_version_integrity(self, session_id: str, version: int) -> bool:
        ...
        try:
            ...
            if not is_valid:
                logger.error(
                    f"Content hash mismatch for version {version}: {current_hash} != {stored_hash}"
                )
    ...
    async def _store_version_metadata(
        ...
        try:
            ...
            await self.state_store.redis.expire(
                metadata_key, self.state_store.session_ttl
            )
    ...
    def _calculate_diff(
        self, from_state: DesignState, to_state: DesignState
    ) -> Dict[str, Any]:
        ...
        for elem_id in set(from_lookup.keys()) & set(to_lookup.keys()):
            ...
            if from_elem != to_elem:
                modified.append({"id": elem_id, "from": from_elem, "to": to_elem})
