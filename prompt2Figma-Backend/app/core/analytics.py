...
from app.core.models import (
    SessionMetrics, EditType, EditContext, SessionMetadata
)
...
class AnalyticsManager:
    ...
    async def track_edit(
        ...
        try:
            ...
            await self.state_store.redis.lpush(
                f"{analytics_key}:processing_times",
                processing_time_ms
            )
            ...
            await self.state_store.redis.lpush(
                f"{session_analytics_key}:processing_times",
                processing_time_ms
            )
            await self.state_store.redis.ltrim(
                f"{session_analytics_key}:processing_times",
    ...
    async def _store_session_metrics(self, session_id: str, metrics: SessionMetrics) -> bool:
        ...
        try:
            ...
            if metrics.user_satisfaction_score is not None:
                metrics_data["user_satisfaction_score"] = metrics.user_satisfaction_score
    ...
    async def get_daily_analytics(self, date: Optional[datetime] = None) -> Dict[str, Any]:
        """
    ...
    async def get_user_engagement_metrics(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            avg_edits_per_session = (
                total_edits / active_sessions
                if active_sessions > 0 else 0.0
            )
            
            avg_duration = (
                total_duration / active_sessions
                if active_sessions > 0 else 0.0
            )
    ...
    async def get_performance_trends(self, days: int = 7) -> Dict[str, Any]:
        ...
        try:
            ...
            for daily_data in analytics_list:
                ...
                avg_processing_times.append(daily_data.get("average_processing_time_ms", 0))
            ...
            overall_avg_time = (
                ...
                if avg_processing_times else 0.0
            )
