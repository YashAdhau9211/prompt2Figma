...
@router.post("/design-sessions/{session_id}/generate-code", response_model=GenerateCodeResponse)
async def generate_code_from_session(session_id: str, version: Optional[int] = None):
    ...
    try:
        ...
        try:
            ...
            design_state = await state_store.get_design_state(session_id, target_version)
            ...
            # Generate React code
            code_task = generate_react_code.apply_async(args=[design_state.wireframe_json])
            react_code: str = code_task.get(timeout=300)
        ...
        raise HTTPException(status_code=500, detail=f"code generation from session failed: {e}")
